...
Renderers.nyserda_slr = {
  ...
  update_legend_url: function (active_layer) {
    var rise = active_layer.parameters.rise;
    var year = active_layer.parameters.year;
    active_layer.legend_url = CDN("http://ciesin.columbia.edu/geoserver/gwc/service/wms?request=GetLegendGraphic&LAYER=nyserda:flood_"+rise+"_"+year+"&format=image/png");
  ...
  create_leaflet_layers: function (map, active_layer) {
    ...
    var active_leaflet_layer = Renderers.nyserda_slr.active_leaflet_layer(map, active_layer);

    if (!active_leaflet_layer) {
        var new_layer =  new L.TileLayer.WMS(CDN("http://ciesin.columbia.edu/geoserver/wms"), {
                layers: "nyserda:flood_"+rise+"_"+year,
                format: 'image/png',
                opacity: 0,
                zIndex: -1,
                transparent: true
  ...
  render: function (map, active_layer, z_index) {
    ...
    Renderers.nyserda_slr.create_leaflet_layers(map, active_layer);
    Renderers.nyserda_slr.update_legend_url(active_layer);
 ...
 /* ,
  get_feature_info_url: function (active_layer) {
    ...
    return CDN("http://ciesin.columbia.edu/geoserver/gwc/service/wms") +
          ...
          "REQUEST=GetFeatureInfo&LAYERS=nyserda:flood_"+rise+"_"+year+"&"+
          "QUERY_LAYERS=nyserda:flood_"+rise+"_"+year+"&"+
          "STYLES=&"+
          "BBOX=<%= bbox %>&"+
          "FEATURE_COUNT=5&"+
          "HEIGHT=<%= height %>&"+
