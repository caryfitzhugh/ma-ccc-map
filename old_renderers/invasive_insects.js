...
Renderers.invasive_insects = {
  ...
  find_geo_json: function (map, active_layer, evt) {
    ...
    var active_svg_layer = Renderers.invasive_insects.active_leaflet_layer(map, active_layer);
  ...
  update_legend: function (map, active_layer) {
    ...
    if (active_leaflet_layer && active_leaflet_layer.color) {
      ...
      active_layer.parameters.legend_text = "Reported Invasive Species (Total Observations)";
    }
  },
  ...
  get_data: function (area, species, callback) {
    ...
    if (Renderers.invasive_insects.cache) {
      callback(null, Renderers.invasive_insects.cache);
    } else {
      $.ajax({
        cache: true,
        url: CDN("http://52.2.5.122:8080/geoserver/www/invasive_insects.json"),
        dataType: "json",
        success: function (data) {
          Renderers.invasive_insects.cache = data;
          callback(null, data);
        },
        error:   function (err) {
          if (err.status !== 200) {
  ...
  create_leaflet_layers: function (map, active_layer) {
    ...
    var active_leaflet_layer = Renderers.invasive_insects.active_leaflet_layer(map, active_layer);
    ...
    if (!active_leaflet_layer) {
      ...
      // Load some Data
      Renderers.invasive_insects.get_data(p.area, p.species,
          ...
          if (error) {
            ...
            GeometryLoader.load(p.area, function (error, geometries) {
              ...
              if (error) {
                ...
                var new_layer = Renderers.invasive_insects.render_data(map,
                                        color,
                                        // "all_insects" / "county" => { "albany" : 4, ....}
                ...
                active_leaflet_layer = Renderers.invasive_insects.active_leaflet_layer(map, active_layer);
  ...
  render: function (map, active_layer, z_index) {
    ...
    Renderers.invasive_insects.create_leaflet_layers(map, active_layer);
    Renderers.invasive_insects.update_legend(map, active_layer);
