Renderers.dow_uncon_aqui_main = {
  ...
  update_legend_url: function (active_layer) {
    ...
    active_layer.legend_url_text = "Click map lines for more info";
  },
  create_leaflet_layers: function (map, active_layer) {
    if (_.isEmpty(active_layer.leaflet_layer_ids)) {
      var layer = new L.TileLayer.WMS(CDN(MAP_SERVER_HOST + "/geoserver/nyccsc/wms"), {
                    layers: 'nyccsc:dow_uncon_aqui_main',
                    format: 'image/png',
                    transparent: true,
                    opacity: 0,
                    zIndex: -1
  ...
  get_feature_info_url: function (active_layer) {
    return CDN(MAP_SERVER_HOST + "/geoserver/nyccsc/wms" +
          ...
          "REQUEST=GetFeatureInfo&LAYERS=nyccsc:dow_uncon_aqui_main&"+
          "QUERY_LAYERS=nyccsc:dow_uncon_aqui_main&"+
          "STYLES=&"+
          "BBOX=<%= bbox %>&"+
          "FEATURE_COUNT=5&"+
          "HEIGHT=<%= height %>&"+
