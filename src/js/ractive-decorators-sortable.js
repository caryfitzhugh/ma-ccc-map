...
(function ( global, factory ) {
  ...
  mousedownHandler = function (e) {
    ...
    if (handle[0].isEqualNode(target)) {
      ...
      sourceArray = sourceKeypath.str.substr( 0, lastDotIndex );
      sourceIndex = +( sourceKeypath.str.substring( lastDotIndex + 1 ) );
	...
	dragenterHandler = function () {
      ...
      // this decorator only works with array members!
      lastDotIndex = targetKeypath.str.lastIndexOf( '.' );

      if ( lastDotIndex === -1 ) {
        throw new Error( errorMessage );
      }

      targetArray = targetKeypath.str.substr( 0, lastDotIndex );
      targetIndex = +( targetKeypath.str.substring( lastDotIndex + 1 ) );
