...
Renderers.narccap_precip = {
  ...
  find_geo_json: function (map, active_layer, evt) {
    ...
    var active_svg_layer = Renderers.narccap_precip.active_leaflet_layer(map, active_layer);
  ...
  clone_layer_name: function (active_layer) {
    var p = _.pick(active_layer.parameters, ["date", "area", "season"]);
  ...
  update_legend: function (map, active_layer) {
    ...
    if (active_leaflet_layer && active_leaflet_layer.color) {
      ...
      active_layer.parameters.legend_text = "Projected Change in Precipitation (cm)";
    }
  },
  ...
  create_leaflet_layers: function (map, active_layer) {
    ...
    var active_leaflet_layer = Renderers.narccap_precip.active_leaflet_layer(map, active_layer);

    if (!active_leaflet_layer) {
      active_layer.leaflet_layer_ids.push(_.merge({}, p, { leaflet_id: "layer-loading"}));

      // Load some Data
      Renderers.narccap.get_narccap_data(p.area, p.season, "pcpn", parseInt(p.date), parseInt(p.date) + active_layer.parameters.date_step,
          ...
          if (error) {
            ...
            GeometryLoader.load(p.area, function (error, geometries) {
              if (error) {
                ...
                active_leaflet_layer = Renderers.narccap_precip.active_leaflet_layer(map, active_layer);
  ...
  render: function (map, active_layer, z_index) {
    ...
    Renderers.narccap_precip.create_leaflet_layers(map, active_layer);
    Renderers.narccap_precip.update_legend(map, active_layer);
