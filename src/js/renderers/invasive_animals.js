...
Renderers.invasive_animals = {
  ...
  find_geo_json: function (map, active_layer, evt) {
    ...
    var active_svg_layer = Renderers.invasive_animals.active_leaflet_layer(map, active_layer);
  ...
  get_data: function (area, species, callback) {
    ...
    if (Renderers.invasive_animals.cache) {
      callback(null, Renderers.invasive_animals.cache);
    } else {
      $.ajax({
        cache: true,
        url: CDN("http://52.2.5.122:8080/geoserver/www/invasive_animals.json"),
        dataType: "json",
        success: function (data) {
          Renderers.invasive_animals.cache = data;
          callback(null, data);
        },
        error:   function (err) {
          if (err.status !== 200) {
  ...
  create_leaflet_layers: function (map, active_layer) {
    ...
    var active_leaflet_layer = Renderers.invasive_animals.active_leaflet_layer(map, active_layer);
    ...
    if (!active_leaflet_layer) {
      ...
      // Load some Data
      Renderers.invasive_animals.get_data(p.area, p.species,
          ...
          if (error) {
            ...
            GeometryLoader.load(p.area, function (error, geometries) {
              ...
              if (error) {
                ...
                var new_layer = Renderers.invasive_animals.render_data(map,
                                        color,
                                        // "all_animals" / "county" => { "albany" : 4, ....}
                ...
                active_leaflet_layer = Renderers.invasive_animals.active_leaflet_layer(map, active_layer);
  ...
  render: function (map, active_layer, z_index) {
    ...
    Renderers.invasive_animals.create_leaflet_layers(map, active_layer);
    Renderers.invasive_animals.update_legend(map, active_layer);
