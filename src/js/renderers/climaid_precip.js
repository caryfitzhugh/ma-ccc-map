...
Renderers.climaid_precip = {
  ...
  find_geo_json: function (map, active_layer, evt) {
    ...
    var active_svg_layer = Renderers.climaid_precip.active_leaflet_layer(map, active_layer);
  ...
  update_legend: function (map, active_layer) {
    ...
    if (active_leaflet_layer && active_leaflet_layer.color) {
      ...
      active_layer.parameters.legend_text = "Precipitation Change (cm)";
  ...
  create_leaflet_layers: function (map, active_layer) {
    ...
    var active_leaflet_layer = Renderers.climaid_precip.active_leaflet_layer(map, active_layer);

    if (!active_leaflet_layer) {
      active_layer.leaflet_layer_ids.push(_.merge({}, p, { leaflet_id: "layer-loading"}));

      // Load some Data
      Renderers.climaid.get_climaid_data(CDN("http://52.2.5.122:8080/geoserver/www/climaid_precip_deltas.json"), p.timestep, p.scenario, p.percentile, p.symbology,
        function (error, geojson, range_extent) {
          if (error) {
            ...
            if (range_extent[0] < 0) {
              ...
              color = color.range(colorbrewer.BuGn[9]);
            ...
            active_leaflet_layer = Renderers.climaid_precip.active_leaflet_layer(map, active_layer);

            active_leaflet_layer.leaflet_id = new_layer._leaflet_id;

            active_leaflet_layer.color = color;

            if (!Renderers.climaid_precip.zoomed_to && !loaded_state) {
              var obj = L.geoJson(geojson);
              map.fitBounds(obj.getBounds());
              Renderers.climaid_precip.zoomed_to = true;
            }
  ...
  render: function (map, active_layer, z_index) {
    ...
    Renderers.climaid_precip.create_leaflet_layers(map, active_layer);
    Renderers.climaid_precip.update_legend(map, active_layer);
