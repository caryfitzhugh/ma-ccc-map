...
RendererTemplates.esri_feature_layer("example_esri_feature_layer", {
  parameters: {
    color_buckets:  [
      {
  ...
  legend_template: `
    <div class='color-block' style="width: {{100.0 / parameters.color_buckets.length}}%;">
          <svg width="100" height="100">
            <rect height="100" width="100" style="fill: {{fill}}; stroke: {{stroke}}; opacity:{{(0.7 * parameters.opacity / 100.0) + 0.3}}; "/>
          </svg>
          <label>{{label}}</label>
    </div>
  `,
