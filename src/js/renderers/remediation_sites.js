...
Renderers.remediation_sites = {
  ...
  update_legend_url: function (active_layer) {
    ...
    active_layer.legend_url_text = "Remediation site boundary (click for info)";
  },
  create_leaflet_layers: function (map, active_layer) {
    if (_.isEmpty(active_layer.leaflet_layer_ids)) {
      var layer = new L.TileLayer.WMS(CDN(MAP_SERVER_HOST + "/geoserver/nyccsc/wms"), {
                  layers: 'nyccsc:remediation_sites',
                  format: 'image/png',
                  transparent: true,
                  opacity: 0,
                  zIndex: -1
  ...
  get_feature_info_url: function (active_layer) {
    return CDN(MAP_SERVER_HOST + "/geoserver/nyccsc/wms" +
          ...
          "REQUEST=GetFeatureInfo&LAYERS=nyccsc:remediation_sites&"+
          "QUERY_LAYERS=nyccsc:remediation_sites&"+
          "STYLES=&"+
          "BBOX=<%= bbox %>&"+
          "FEATURE_COUNT=5&"+
          "HEIGHT=<%= height %>&"+
