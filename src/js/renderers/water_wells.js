...
Renderers.water_wells = {
  pickle: function (al) {
    delete al.legend_url;
    delete al.legend_url_text;
  },
  update_legend_url: function (active_layer) {
    active_layer.legend_url = CDN(MAP_SERVER_HOST + "/geoserver/wms?request=GetLegendGraphic&LAYER=nyccsc:waterwellprogram&format=image/png");
  },
  create_leaflet_layers: function (map, active_layer) {
    if (_.isEmpty(active_layer.leaflet_layer_ids)) {
      var layer = new L.TileLayer.WMS(CDN(MAP_SERVER_HOST + "/geoserver/nyccsc/wms"), {
                  layers: 'nyccsc:waterwellprogram',
                  format: 'image/png',
                  transparent: true,
                  opacity: 0,
                  zIndex: -1
  ...
  get_feature_info_url: function (active_layer) {
    return CDN(MAP_SERVER_HOST + "/geoserver/nyccsc/wms" +
          ...
          "REQUEST=GetFeatureInfo&LAYERS=nyccsc:waterwellprogram&"+
          "QUERY_LAYERS=nyccsc:waterwellprogram&"+
          "STYLES=&"+
          "BBOX=<%= bbox %>&"+
          "FEATURE_COUNT=5&"+
          "HEIGHT=<%= height %>&"+
