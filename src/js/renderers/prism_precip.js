...
Renderers.prism_precip = {
  ...
  find_geo_json: function (map, active_layer, evt) {
    ...
    var active_svg_layer = Renderers.prism_precip.active_leaflet_layer(map, active_layer);
  ...
  clone_layer_name: function (active_layer) {
    var p = _.pick(active_layer.parameters, ["area", "season"]);
    var layer_default = _.find(LayerInfo, {id: active_layer.layer_default_id});
    var name =  layer_default.name +  " S:" + p.season + " by " + p.area;
    return name;
  },
  update_legend: function (map, active_layer) {
    ...
    if (active_leaflet_layer && active_leaflet_layer.color) {
      ...
      active_layer.parameters.legend_text = "Precipitation (Inches)";
    }
  },
  ...
  create_leaflet_layers: function (map, active_layer) {
    ...
    var active_leaflet_layer = Renderers.prism_precip.active_leaflet_layer(map, active_layer);

    if (!active_leaflet_layer) {
      active_layer.leaflet_layer_ids.push(_.merge({}, p, { leaflet_id: "layer-loading"}));

      // Load some Data
      Renderers.prism.get_prism_data(p.area, p.season, "pcpn", 1980, 2014,
          ...
          if (error) {
            ...
            GeometryLoader.load(p.area, function (error, geometries) {
              if (error ) {
                ...
                active_leaflet_layer = Renderers.prism_precip.active_leaflet_layer(map, active_layer);
  ...
  render: function (map, active_layer, z_index) {
    ...
    Renderers.prism_precip.create_leaflet_layers(map, active_layer);
    Renderers.prism_precip.update_legend(map, active_layer);
