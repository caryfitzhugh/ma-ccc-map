...
*   Apache License
*
*  MODIFICATIONS BY Cary Fitzhugh
*   -- add (g|s)etZIndex methods to set the index on the image of a dynamicMapLayer
*   -- add nyccsc-loaded/error events for dynamic-map-layer loading (so we know when they load / don't)
*   -- add CDN to run the resulting URLs through the CDN (line 2156)
*
*   */
(function (factory) {
...
EsriLeaflet.Layers.RasterLayer =  L.Class.extend({
  ...
  setZIndex: function(zIndex){
    ...
    if (this._currentImage) {
      this._currentImage._image.style.zIndex = zIndex;
    }
    return this;
  },
  ...
  _renderImage: function(url, bounds){
    if(this._map){
      ...
      // once the image loads - and has an error
      image._image.onerror = function (e) {
        this.fire('nyccsc-error');
      }.bind(this);

      image.once("load", function (e) {
        this.fire('nyccsc-loaded');
      }.bind(this));
  ...
  _requestExport: function (params, bounds) {
    if(this.options.f === 'json'){
      this._service.request('export', params, function(error, response){
        if(error) { return; } // we really can't do anything here but authenticate or requesterror will fire
        this._renderImage(CDN(response.href), bounds);
      }, this);
    } else {
      params.f = 'image';
