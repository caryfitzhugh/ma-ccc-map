...
L.D3SvgOverlay = (L.version < "1.0" ? L.Class : L.Layer).extend({
    ...
    createSVG: function () {
      ...
      var new_svg = L.Path.prototype._createElement('svg');
      this.map._panes.overlayPane.appendChild(new_svg);
      // Always hide on Zooming.
      L.DomUtil.addClass(new_svg, 'leaflet-zoom-animated');

      this.map.on({
        'zoomanim': this._animatePathZoom.bind(this),
        'zoomend': this._endPathZoom.bind(this),
        'moveend': this._updateSvgViewport.bind(this)
        });

      return d3.select(new_svg);
    },
    onAdd: function (map) {
        this.map = map;
        var _layer = this;
        ...
        this._updateSvgViewport.bind(this)();
    ...
    getZoomScale: function (toZoom) {
      ...
      return crs.scale(toZoom) / crs.scale(this.map._zoom);
    },

    // layer point of the current center
    getCenterLayerPoint: function () {
      return this.map.containerPointToLayerPoint(this.map.getSize()._divideBy(2));
    },

    // offset of the specified place to the current center in pixels
    getCenterOffset: function (latlng) {
      return this.map.latLngToLayerPoint(latlng).subtract(this.getCenterLayerPoint());
    },

   _animatePathZoom: function (e) {

     var scale = this.getZoomScale(e.zoom),
         offset = this.getCenterOffset(e.center)._multiplyBy(-scale)._add(this.map._pathViewport.min),
         transform_str = L.DomUtil.getTranslateString(offset) + ' scale(' + scale + ') ' ;

     this._svg[0][0].style[L.DomUtil.TRANSFORM] = transform_str;
   ...
   _endPathZoom: function () {
    ...
    var vp = this.map._pathViewport,
        min = vp.min,
        max = vp.max,
        width = max.x - min.x,
        height = max.y - min.y,
        // root is a d3 selected result, so it is an array of arrays, pull out the DOM element
        root = (this._svg && this._svg[0] && this._svg[0][0]),
        pane = this.map._panes.overlayPane;
  ...
  setOpacity: function (opac) {
    this._svg[0][0].style.opacity = opac;
  },
  setZIndex: function (zi) {
    this._svg[0][0].style.zIndex = zi;
  },
  find_polygon_with_click: function (evt) {
    ...
    var match = _.find(paths, function (path) {
      var result = null;
      //var path_points = path.getPathData();
      var path_points = JSON.parse(path.getAttribute('data-lat-lng-path'));

      return gju.pointInPolygon(point_to_check, path_points);
    });
    return match;
  }
});
